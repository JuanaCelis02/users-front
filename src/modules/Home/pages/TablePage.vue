<template>
	<div class="container">
		...
		<div class="d-flex justify-content-end my-2">
			<button class="btn btn-success m-2" @click="emailDrawer = true">
				Enviar Email
			</button>
			<button class="btn btn-primary m-2" @click="drawer = true">
				Crear Usuario
			</button>
		</div>
		...
		<div class="d-flex justify-content-end">
			<el-pagination class="my-3" :current-page="currentPage" :page-size="pageSize" :total="totalUsers"
				@current-change="handlePageChange" />
		</div>
	</div>

	<el-drawer v-model="emailDrawer" @close="cancelDrawer">
		<template #header>
			<h2>Automatizar Email</h2>
		</template>
		<template #default>
			<h4>Ajustar hora</h4>
			<div class="row mb-3">
				<div class="col">
					<el-input v-model="formData.hour" class="w-100" size="large" placeholder="00:00:00" />
				</div>
				<div class="col">
					<el-button class="w-100" type="primary" plain size="large" @click="enviarCorreosSuperadmins">
						Actualizar
					</el-button>
				</div>
			</div>
	...
	<el-drawer v-model="drawer" @close="cancelDrawer">
		...
		<template #default>
			...
			<div class="row mb-3">
				<div class="col">
					<el-input v-model="formData.name" class="w-100" size="large" placeholder="Ingrese Nombre" />
				</div>
				<div class="col">
					<el-input v-model="formData.lastName" class="w-100" size="large" placeholder="Ingrese Apellido" />
				</div>
			</div>
			<div class="row mb-3">
				<div class="col">
					<el-input v-model="formData.numberDocument" class="w-100" type="number" size="large"
						placeholder="Ingrese Numero de documento" />
				</div>
				<div class="col">
					<el-input v-model="formData.phoneNumber" class="w-100" type="phone" size="large"
						:formatter="formatPhoneNumber" :parser="parsePhoneNumber" maxlength="10"
						placeholder="(123) 456 7890" />
				</div>
			</div>
			<div class="row mb-3">
				<div class="col">
					<el-input v-model="formData.email" class="w-100" type="email" size="large"
						placeholder="Ingrese Correo" />
				</div>
				<div class="col">
					<el-input v-model="formData.bithDate" class="w-100" type="date" size="large"
			...
			<div class="row mb-3">
				<div class="col">
					<el-input v-model="formPassword.password1" class="w-100" size="large" type="password" show-password
						placeholder="Ingrese Contraseña" />
				</div>
				<div class="col">
					<el-input v-model="formPassword.password2" class="w-100" size="large" type="password"
						@input="validatePasswordMatch" show-password placeholder="confirmar Contraseña" />
				</div>
			...
			<div class="row mb-3">
				<div class="col">
					<el-select v-model="role" class="w-100" placeholder="Asignar Role" size="large">
						<el-option v-for="item in roles" :key="item.id" :label="item.roleName" :value="item.id" />
					</el-select>
				</div>
				<div class="col">
					<el-button class="w-100" type="primary" plain size="large" @click="dialogVisible = true">Crear Nuevo
						Role</el-button>
				</div>
			</div>
			<h4>Avatar</h4>
	...
	<el-dialog v-model="dialogVisible" title="Crear Nuevo Role" width="40%" :before-close="closeDialog">
		<div class="row mb-3">
			<div class="col">
				<el-input v-model="formRole.roleName" class="w-100" size="large" placeholder="Nombre Role" />
			</div>
			<div class="col"></div>
		</div>
		<div class="row mb-3">
			<div class="col">
				<el-input v-model="formRole.descriptionRole" :rows="2" type="textarea" placeholder="Ingrese Descripción" />
			</div>
			<div class="col"></div>
		</div>
		<div class="mb-3 d-flex flex-column">
			<h6>Estado del Role</h6>
			<el-switch v-model="formRole.statusRole" :on-value="true" :off-value="false"
...
export default {
	...
	methods: {
		...
		async enviarCorreosSuperadmins() {
			try {
				// Obtener la hora deseada del formulario
				const horaEnvioCorreo = this.formData.hour;
				...
				console.log('Respuesta de envío de correos en el componente:', response);
			} catch (error) {
				console.error('Error al enviar correos desde el componente:', error);
